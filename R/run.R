...
#'
#'
#' @param k \code{integer} vector
#' @param n \code{integer} length of code{x}
#' @param var \code{character} name of the variable
check_k <- function(k, n, var) {
  length_k <- length(k)

  if (length_k != n && length_k > 1) {
    stop(
  ...
  } else if (length_k > 1 && any(k < 0)) {
    stop("k can't be negative")
  }
}

#' checks if type of \code{x} is valid
#'
#' @param x variable
check_type <- function(x) {
  if (!is(x, "character") &&
      ...
      !is(x, "logical")) {
    stop(
      sprintf("Invalid output type (%s). Please specify one of the following:
              'character', 'numeric', 'integer', 'logical'", typeof(x)),
      call. = FALSE
    )
  }
}
