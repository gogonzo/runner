...
tinytest::expect_equal(out, test)
...
# Errors-----
tinytest::expect_error(which_run(x1, which = "any"),
             "which value should be either")
tinytest::expect_error(which_run(x1, k = (1:999)),
             "length of k and length of x differs")
tinytest::expect_error(which_run(x1, k = c(NA, k[-1])),
             "Function doesn't accept NA values in k vector")

tinytest::expect_error(which_run(x1, lag = (1:99)),
             "length of lag and length of x differs")
tinytest::expect_error(which_run(x1, lag = c(NA, lag[-1])),
             "Function doesn't accept NA values in lag vector")

tinytest::expect_error(which_run(x1, idx = (1:99)),
             "length of idx and length of x differs")
tinytest::expect_error(which_run(x1, idx = c(NA, 1:99)),
