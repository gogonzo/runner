...
### Parallel mode
User is able to calculate rolling windows in parallel mode. To do so, one has to
prepare a cluster and pass it to the runner function through `cl` argument.
...
*Be aware that in many cases parallel mode might not be quicker than a single-thread*
*mode. It's up to user to assess whether it's relevant to parallelize process.*
