...
todo:
 - aggregate to custom time-interval
 - 
...
### Prediction on current day based on preeceding 2-weeks regression
...
data$pred <- runner(
  ...
  k = "2 weeks",
  idx = data$date,
  f = running_regression <- function(data) {
      predict(
        lm(y ~ x, data = data)
      )[nrow(data)]
  }
...
df2 %>%
  ...
  mutate(
    ...
    slope = runner(
      ...
      lag = "5 days",
      idx = df$Date,
      at = Event_date[df$Company.name == Company.name2[1]],
      company = Company.name2[1],
      f = function(x) {
        coef(
          lm(stock_return ~ market_return, data = x)
        )[2]
