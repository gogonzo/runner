...
template <int RTYPE>
List window_simple(Rcpp::Vector<RTYPE> const& x,
...
SEXP window_run(SEXP x,
  ...
  switch (TYPEOF(x)) {
    case INTSXP:  return window_simple(as<IntegerVector>(x),   k, lag, idx, na_pad);
    case REALSXP: return window_simple(as<NumericVector>(x),   k, lag, idx, na_pad);
    case STRSXP:  return window_simple(as<CharacterVector>(x), k, lag, idx, na_pad);
    case LGLSXP:  return window_simple(as<LogicalVector>(x),   k, lag, idx, na_pad);
    case CPLXSXP: return window_simple(as<ComplexVector>(x),   k, lag, idx, na_pad);
    default: {
